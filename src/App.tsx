...
export default function App() {
    ...
    try {
      ...
      if (saved) {
        ...
        for (const k in parsed) {
          ...
          // Accept data URLs, http(s) URLs, or relative paths like /logos/...
          if (val && typeof val === 'string' && (val.startsWith('data:') || val.startsWith('http:') || val.startsWith('https:') || val.startsWith('/') || val.startsWith('.'))) {
  ...
  const saveCustomLogos = async () => {
    setIsSavingLogos(true);
    const updatedLogos = { ...tempLogos };

    try {
      for (const key of Object.keys(tempLogos) as (keyof CustomLogos)[]) {
        const val = tempLogos[key];
        if (val && val.startsWith('data:')) {
          try {
            const res = await fetch('/api/save-logo', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ key, dataUrl: val })
            });
            if (res.ok) {
              const data = await res.json();
              if (data.logoUrl) {
                updatedLogos[key] = data.logoUrl;
              }
            }
          } catch (err) {
            console.error(`Error saving logo for ${key} to server:`, err);
          }
        }
      }

      setLogos(updatedLogos);
      setTempLogos(updatedLogos);
      try {
        localStorage.setItem('arkos_custom_logos', JSON.stringify(updatedLogos));
      } catch (e) {
        console.error('Error saving custom logos to localStorage:', e);
      }
    } finally {
      ...
      <header className="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="mt-6 pt-5 border-t border-slate-100 text-center md:text-left flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            {/* Coopercity client badge */}
            <div className="flex items-center gap-2.5 self-center md:self-auto bg-slate-100 p-1.5 px-3 rounded-xl border border-slate-200">
            ...
            <div className="space-y-5">
              ...
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-slate-800 flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-[#0E6135]"></span>
                    Logo COOPERCITYSP (Cliente)
                  </label>
                  {tempLogos.coopercitysp ? (
                    <img src={tempLogos.coopercitysp} alt="Preview Coopercity" className="h-6 w-auto max-w-[100px] object-contain border border-slate-200 bg-white p-1 rounded" />
                  ) : (
                    <span className="text-[10px] text-slate-400 italic">Sem imagem</span>
                  )}
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <input
                    type="text"
                    placeholder="URL do logotipo COOPERCITYSP..."
                    value={tempLogos.coopercitysp}
                    onChange={e => handleLogoChange('coopercitysp', e.target.value)}
                    className="w-full px-3 py-1.5 text-xs bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#0E6135]"
                  />
                  <input
                    type="file"
                    accept="image/*"
                    onChange={e => handleFileUpload('coopercitysp', e)}
                    className="w-full text-xs text-slate-500 file:mr-2 file:py-1 file:px-2.5 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-emerald-50 file:text-[#0E6135] hover:file:bg-emerald-100 cursor-pointer"
                  />
                </div>
              </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-slate-100">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-[#0E6135] hover:bg-[#073B20] disabled:opacity-50 text-white font-bold text-xs rounded-xl transition-colors cursor-pointer shadow-2xs flex items-center gap-2"
                >
                  {isSavingLogos ? (
                    <>
